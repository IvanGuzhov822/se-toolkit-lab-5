...
async def get_scores(
    ...
    # Query interactions and bucket them
    score_bucket = case(
...
async def get_pass_rates(
    ...
    for task in tasks:
        ...
        interactions_stmt = select(
            func.avg(InteractionLog.score).label("avg_score"),
            func.count().label("attempts")
        ).where(
            InteractionLog.item_id == task.id
        )
        interactions_result = await session.exec(interactions_stmt)
        row = interactions_result.first()

        if row and row[1] > 0:
            avg_score = round(float(row[0]), 1) if row[0] is not None else 0.0
            result.append({
                "task": task.title,
                "avg_score": avg_score,
                "attempts": row[1],
